...
import model.AbstractJoinPoint;
import model.Attribute;
import model.Constructor;
import model.ConstructorCall;
import model.ConstructorExecution;
import model.Event;
import model.FieldRead;
import model.FieldWrite;
import model.IModel;
import model.Instance;
import model.JoinPointContextInformation;
import model.Method;
import model.MethodCall;
import model.MethodExecution;
import model.Model;
import model.MyWeakReference;
import model.SintaticStructure;
import model.Snapshot;
import model.StaticInitializer;
import model.StaticInitialization;
import model.ThreadFlow;
...
aspect Tracer {
	...
	pointcut codeInsideMyProject() : within(aspects..* || model..* || controller..* || patterns..* || view..* || utils..*) || cflow(execution(String *.toString()));
		...
		if(thiz!=null)
		{
			...
			for (Field field : fields) {
				...
				if(object!=null)
				{
					...
					fieldOfObject = new model.Field(field.getName(),object.toString(), field.getGenericType().toString(), field.getModifiers(), instanceOfObject);
